...
const useTrainStatusLayers = (enable: boolean) => {
    ...
    () => [
      ...tainanStations
        .map((t, i) => {
          ...
          return createBubble({
            current: Math.floor(
              capacity * 0.5 + Math.random() * capacity * 0.9 - capacity * 0.45,
            ),
            max: capacity,
            maxRadius: capacity,
  ...
  const [data, setData] = useState<any>(createData());
  ...
  return [
    new ScatterplotLayer({
      ...
      transitions: {
        ...
        getRadius: intervalMs / 2,
        getFillColor: intervalMs / 2,
      },
    }),
    new TextLayer({
...
export default useTrainStatusLayers;
